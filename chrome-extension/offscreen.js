...
function state(){return {settings,enabled,gain:enabled?envelope.gain:1,phase:enabled?envelope.phase:"ready",sources:[...sources.values()].map(s=>({tabId:s.tabId,title:s.title,role:s.role,peak:s.peak||0}))};}
  ...
  switch(m.type){
    ...
    case "ADD": {
      ...
      try {
        ...
        sources.set(m.tabId,{tabId:m.tabId,title:m.title,role:m.role,stream,input,analyser,gain,samples:new Float32Array(analyser.fftSize)});
