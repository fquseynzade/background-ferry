...
internal static class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        try { UiText.Apply(Storage.LoadSettings().Language); } catch { UiText.Apply("ru"); }
        string instance = stateIndex >= 0 ? "." + Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(Storage.DirectoryPath.ToUpperInvariant())))[..16] : "";
        using var mutex = new Mutex(true, @"Local\BackgroundFerry.Desktop" + instance, out bool first);
        if (!first) { MessageBox.Show(UiText.T("AlreadyRunning"), "Background Ferry"); return 0; }
        MainWindow? window = null;
        app.DispatcherUnhandledException += (_, e) =>
        {
            Storage.Log(e.Exception);
            try { window?.EmergencyStop(); } catch (Exception stopError) { Storage.Log(stopError); }
            MessageBox.Show(UiText.T("ErrorStopped") + "\n\n" + e.Exception.Message, "Background Ferry");
            e.Handled = true;
        };
        try
        {
            using var recoveryGate = new RecoveryGate(6000);
            if (!recoveryGate.Acquired) throw new InvalidOperationException(UiText.T("RecoveryBusy"));
        ...
        catch (Exception ex) { Storage.Log(ex); MessageBox.Show(ex.Message, UiText.T("CannotStart")); return 1; }
    }
}
